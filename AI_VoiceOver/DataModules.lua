...
DataModules =
{
    ...
    availableModules = {
        {
            AddonName = "Updates by Aelak",
            Title = "Updates by Aelak!  Maybe....",
            ContentVersion = "0.1",
            RelevantAboveVersion = 0,
            URL = "discord.gg/jHPsF2dt9y",
        },
    },
}
...
{
    [Enums.SoundEvent.QuestAccept]   = function(soundData)
        Debug:Print("GUID: " .. (soundData.unitGUID or "nil"), "QuestAccept")
        local npcId = soundData.unitGUID and Utils:GetIDFromGUID(soundData.unitGUID)
        if npcId then
            Debug:Print("Interacting with NPCID: " .. npcId, "QuestAccept")
            local specificFileName = format("%d-accept-%d", soundData.questID, npcId)
            Debug:Print("Searching for specific format: " .. specificFileName, "QuestAccept")
            ...
            for _, module in DataModules:GetModules() do
                ...
                if data then
                    local length = data[specificFileName]
                    if length then
                        Debug:Print("Found specific format: " .. specificFileName, "QuestAccept")
                        return specificFileName
                    end
                end
            end
            Debug:Print("No specific format found, using generic format", "QuestAccept")
        else
            Debug:Print("No NPCID found in GUID", "QuestAccept")
        end
        return format("%d-%s", soundData.questID, "accept")
    end,
    [Enums.SoundEvent.QuestProgress] = function(soundData)
        Debug:Print("GUID: " .. (soundData.unitGUID or "nil"), "QuestProgress")
        local npcId = soundData.unitGUID and Utils:GetIDFromGUID(soundData.unitGUID)
        if npcId then
            Debug:Print("Interacting with NPCID: " .. npcId, "QuestProgress")
            local specificFileName = format("%d-progress-%d", soundData.questID, npcId)
            Debug:Print("Searching for specific format: " .. specificFileName, "QuestProgress")
            ...
            for _, module in DataModules:GetModules() do
                ...
                if data then
                    local length = data[specificFileName]
                    if length then
                        Debug:Print("Found specific format: " .. specificFileName, "QuestProgress")
                        return specificFileName
                    end
                end
            end
            Debug:Print("No specific format found, using generic format", "QuestProgress")
        else
            Debug:Print("No NPCID found in GUID", "QuestProgress")
        end
        return format("%d-%s", soundData.questID, "progress")
    end,
    [Enums.SoundEvent.QuestComplete] = function(soundData)
        Debug:Print("GUID: " .. (soundData.unitGUID or "nil"), "QuestComplete")
        local npcId = soundData.unitGUID and Utils:GetIDFromGUID(soundData.unitGUID)
        if npcId then
            Debug:Print("Interacting with NPCID: " .. npcId, "QuestComplete")
            local specificFileName = format("%d-complete-%d", soundData.questID, npcId)
            Debug:Print("Searching for specific format: " .. specificFileName, "QuestComplete")
            ...
            for _, module in DataModules:GetModules() do
                ...
                if data then
                    local length = data[specificFileName]
                    if length then
                        Debug:Print("Found specific format: " .. specificFileName, "QuestComplete")
                        return specificFileName
                    end
                end
            end
            Debug:Print("No specific format found, using generic format", "QuestComplete")
        else
            Debug:Print("No NPCID found in GUID", "QuestComplete")
        end
...
function DataModules:PrepareSound(soundData)
    -- Only determine filename if not already provided
    if not soundData.fileName then
        Debug:Print("PrepareSound: fileName not provided, determining using getFileNameForEvent for event: " .. (Enums.SoundEvent:GetName(soundData.event) or soundData.event), "DataModules")
        soundData.fileName = getFileNameForEvent[soundData.event](soundData)
    else
        Debug:Print("PrepareSound: fileName WAS provided: " .. soundData.fileName, "DataModules")
    end

    if soundData.fileName == nil then
        Debug:Print("PrepareSound: fileName is nil, returning false.", "DataModules")
        return false
    end

    for _, module in self:GetModules() do
        Debug:Print("PrepareSound: Checking module " .. module.METADATA.AddonName .. " for file: " .. soundData.fileName, "DataModules")
        local data = module.SoundLengthLookupByFileName
        if data then
            local playerGenderedFileName = DataModules:AddPlayerGenderToFilename(soundData.fileName)
            Debug:Print("PrepareSound: Checking gendered: " .. playerGenderedFileName .. " and non-gendered: " .. soundData.fileName, "DataModules")
            local length = data[playerGenderedFileName]
            if length then
                Debug:Print("PrepareSound: Found gendered match: " .. playerGenderedFileName .. " with length " .. length, "DataModules")
                soundData.fileName = playerGenderedFileName
            else
                length = data[soundData.fileName]
                if length then
                    Debug:Print("PrepareSound: Found non-gendered match: " .. soundData.fileName .. " with length " .. length, "DataModules")
                end
            end
            if length then
                ...
                Debug:Print("PrepareSound: Success! Found file in module " .. module.METADATA.AddonName .. ". Path: " .. soundData.filePath .. " Length: " .. soundData.length, "DataModules")
                return true
            end
        end
    end

    Debug:Print("PrepareSound: Failed to find filename \"" .. soundData.fileName .. "\" in any loaded module's SoundLengthLookupByFileName.", "DataModules")
    return false
end
...
--- Checks if any sound file exists for a given quest and event (generic or specific with any NPCID)
--- Used by Quest Tracker to enable the play button even if only specific NPCID version exists.
---@param questID number
---@param event SoundEvent
---@return boolean found
function DataModules:DoesQuestSoundExist(questID, event)
    local eventStringMap = {
        [Enums.SoundEvent.QuestAccept]   = "accept",
        [Enums.SoundEvent.QuestProgress] = "progress",
        [Enums.SoundEvent.QuestComplete] = "complete",
        -- Only QuestAccept seems needed by the tracker button currently
    }
    local eventString = eventStringMap[event]
    if not eventString then
        Debug:Print("DoesQuestSoundExist called with unhandled event: " .. (Enums.SoundEvent:GetName(event) or event), "DataModules")
        return false
    end

    local basePrefix = format("%d-%s", questID, eventString) -- e.g., "9279-accept"

    for _, module in self:GetModules() do
        if module.SoundLengthLookupByFileName then
            for filename, _ in pairs(module.SoundLengthLookupByFileName) do
                local cleanFilename = filename:gsub("^[mf]%-", "") -- Remove potential gender prefix
                -- Check if the clean filename matches the generic pattern (e.g., "9279-accept")
                -- or starts with the specific pattern (e.g., "9279-accept-")
                if cleanFilename == basePrefix or cleanFilename:find(basePrefix .. "-", 1, true) == 1 then
                    return true -- Found a matching file (either generic or specific)
                end
            end
        end
    end

    -- Debug:Print("DoesQuestSoundExist found no match for QuestID: " .. questID .. ", Event: " .. eventString, "DataModules")
    return false -- No matching file found in any module
end

--- Finds the exact filename of the first specific quest sound file (questid-event-npcid.mp3) found for a given quest/event.
--- Iterates through modules respecting priority.
--- Returns the full filename (including potential gender prefix) or nil if none found.
---@param questID number
---@param event SoundEvent
---@return string? filename
function DataModules:FindFirstSpecificQuestSoundFile(questID, event)
    local eventStringMap = {
        [Enums.SoundEvent.QuestAccept]   = "accept",
        [Enums.SoundEvent.QuestProgress] = "progress",
        [Enums.SoundEvent.QuestComplete] = "complete",
    ...
    local eventString = eventStringMap[event]
    if not eventString then return nil end

    local specificPrefix = format("%d-%s-", questID, eventString) -- e.g., "9279-accept-"

    for _, module in self:GetModules() do -- Iterates respecting priority
        if module.SoundLengthLookupByFileName then
            -- We need a consistent order to pick the *first* match reliably if multiple exist within the *same* module
            -- Sorting keys might be slow if done often, but necessary for determinism.
            local sortedFilenames = {}
            for filename in pairs(module.SoundLengthLookupByFileName) do
                table.insert(sortedFilenames, filename)
            end
            table.sort(sortedFilenames)

            for _, filename in ipairs(sortedFilenames) do
                local cleanFilename = filename:gsub("^[mf]%-", "") -- Remove potential gender prefix for matching
                if cleanFilename:find(specificPrefix, 1, true) == 1 then
                    -- Found a specific file matching the pattern
                    Debug:Print("FindFirstSpecificQuestSoundFile found: " .. filename .. " for QuestID: " .. questID .. " Event: " .. eventString, "DataModules")
                    return filename -- Return the original filename (with prefix)
                end
            end
        end
    end

    -- Debug:Print("FindFirstSpecificQuestSoundFile found NO specific match for QuestID: " .. questID .. ", Event: " .. eventString, "DataModules")
    return nil -- No specific file found in any module
end
